...
namespace PeoManageSoft.Business.Domain.Services.Functions.Title.NameExists
{
    ...
    internal sealed class NameExistsFunction : INameExistsFunction
    {
        ...
        /// <summary>
        /// Initializes a new instance of the PeoManageSoft.Business.Domain.Services.Functions.Title.NameExists.NameExistsFunction class.
        /// </summary>
        ...
        public async Task<bool> ExecuteAsync(NameExistsFunctionRequest request)
        {
            ...
            var rules = new List<IRule<TitleEntityField>>
                {
                    _repositoryFactory.CreateRule(TitleEntityField.Name, SqlComparisonOperator.EqualTo, request.Name)
                };

            if (request.TitleId.HasValue)
            {
                rules.Add(_repositoryFactory.CreateRule(TitleEntityField.Id_Readonly, SqlComparisonOperator.NotEqualTo, request.TitleId.Value, SqlOperator.And));
            }
